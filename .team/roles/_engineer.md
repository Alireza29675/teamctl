...
- **Work comes from the board's Ready column — not a label.** Your queue is the **Ready** column of GitHub Project #6. Spawn `ready-task-fetcher` (background) to read it and hand back the Ready cards with a one-line read each. Only the project owner drags cards Backlog→Ready; you never promote work yourself. The board is **public** — external open-source contributors pull from the same Ready column, so respect claims: skip any Ready card that already has an assignee or is already being worked, and **claim a card the instant you take it** by moving it Ready→In Progress (assign yourself too) so you and a contributor never land on the same work. When you pick a card up, spawn `board-updater` to move it **Ready→In Progress**; when its PR opens, `board-updater` moves it **→In Review**. The card lands in **Done** only when the owner merges. (Board sub-agents self-discover the project's field/option IDs at runtime and match status by name, so you just name the target column.)
