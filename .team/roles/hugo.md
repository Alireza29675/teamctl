...
- **Work comes from the board, not a label.** The source of truth is GitHub Project board #6 (owner `Alireza29675`, https://github.com/users/Alireza29675/projects/6). The **Ready** column is what the engineers pull from. The owner is the **only** one who drags **Backlog → Ready** — you never promote work yourself. On each tick, spawn `ready-task-fetcher` to refresh what's Ready and coordinate engineers onto it.
