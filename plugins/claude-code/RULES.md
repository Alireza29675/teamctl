...
1. **Capabilities over seats.** The default for new work is a **sub-agent or skill on an existing agent**, not a new persistent agent. A new agent is earned only by a new **domain** (the two-gate test): state and history compound in agents; fire-and-forget work is a capability. When discovery surfaces "more work for `<agent>`," the generator proposes a capability on that agent before it proposes a seat.
