...
2. **The capability stack — per agent.** For each proposed agent, name its stack from [capability-catalog.md](../capability-catalog.md), picked by the agent's domain and shape (builder vs. compass):
   ...
   **Capabilities over new sessions (the lean default).** A persistent agent is a **parallel agent session for separate focus** — earned by a domain, not a headcount seat. When discovery surfaced something that's really *more work for an existing session*, the proposal says so: *"that's a sub-agent/skill on `<agent>`, not a new session."* A new session is earned only by separate focus — a new **domain** (the two-gate test); fire-and-forget work is a capability, not a parallel session.
