...
Given a PR (number or url), you read it for real: `gh pr view <n>`, `gh pr diff <n>`, the linked issue or `requirements.md` item, and the CI status (`gh pr checks <n>`). Then you produce a summary a smart non-engineer fully understands:
