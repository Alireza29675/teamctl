...
teamctl is opinionated about its building blocks — persistent agents with their own identity and memory, durable mailboxes, supervised processes, channels with ACLs, a `reports_to` hierarchy. Those are the primitives. They shape what kind of work you can do with the tool.
