...
| `on_rate_limit` | list | global default | Overrides the global rate-limit hook chain for this agent. Each entry is the `name` of a hook defined under the global `rate_limits.hooks`. Unset → the global `rate_limits.default_on_hit` chain (which is `[wait]` when empty). |
...
| `subagents` | list | `[]` | Per-agent Claude Code sub-agents. Each entry is a compose-root-relative path to a sub-agent markdown file (frontmatter `name` / `description` / optional `tools` / `model`; body → its system prompt). Passed via Claude Code's `--agents`, additively on top of the project `.claude/agents/`. Claude Code agents only; on other runtimes it renders nothing and logs a warning. |
