...
mod tests {
    ...
    fn upsert_manager_telegram_succeeds_for_consecutive_managers_in_same_project() {
        ...
        // The old project-lookup re-read each candidate file from
        ...
        // manager's lookup hit no match and bailed with
        // "project not found in compose" — even though both managers
        // lived in the file the loop had just edited.
        //
        // This test pins the second-iteration shape: build a minimal
        // .team/ with two managers in one project, load the compose,
        // call upsert for each manager in sequence, and assert both
        // calls succeed AND the file ends with both telegram blocks.
        let dir = tempfile::tempdir().unwrap();
        ...
        // First iteration — succeeds today + post-fix.
        upsert_manager_telegram(&compose, "p:alpha", "ALPHA_TOKEN", "ALPHA_CHATS")
            .expect("first manager upsert succeeds");

        // Second iteration — the pre-fix bug: the find-closure
        // silently fails to parse the just-edited file, lookup
        // returns None, "project `p` not found in compose" bails.
